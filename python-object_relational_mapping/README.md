...
In the __first part__, you will use the module `MySQLdb` to connect to a MySQL database and execute your SQL queries.

In the __second part__, you will use the module `SQLAlchemy` (don’t ask me how to pronounce it…) an Object Relational Mapper (ORM).
...
Once again, write a script that takes in arguments and displays all values in the    table of    where   matches the argument. But this time, write one that is safe from MySQL injections!
...
* Your script should connect to a MySQL server running on `localhost` at port `3306`
* __WARNING:__ all classes who inherit from `Base` __must__ be imported before calling `Base.metadata.create_all(engine)`
...
Write a script that deletes all `State` objects with a name containing the letter `a` from the database  `hbtn_0e_0_usa`:
